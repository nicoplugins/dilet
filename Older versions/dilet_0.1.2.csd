...
button bounds(425, 190, 40, 20), channel("filter"), text("Filter"), , colour:1(255, 165, 0, 255), outlinecolour(0, 0, 0, 0), value(1)
...
opcode  filters, aa, aa
    ...
    if  kChange==1    then
        if      kLPCf=$HITHRESH && kComp[0]>kComp[1]    then 
        
        elseif  kLPCf=$HITHRESH && kComp[0]<kComp[1]    then
        
            chnset  kHPLimitMinus, "HPF"
            chnset  kLPLimitMinus, "LPF"
        
        elseif  kHPCf=$LOWTHRESH && kComp[0]>kComp[1]   then
        ...
        elseif  kHPCf=$HITHRESH && kComp[0]>kComp[1]    then
        ...
        elseif  kLPCf=$LOWTHRESH && kComp[0]>kComp[1]   then
...
instr		ModDelay
	...
	*/
    ...
    if  kFiltButt==1  then
	        aDelL, aDelR filters aDelL, aDelR
	endif
